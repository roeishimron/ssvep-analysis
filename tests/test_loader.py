...
# Mock mne before importing loader
mock_mne = MagicMock()
...
class TestStudyLoader(unittest.TestCase):
    ...
    def test_parse_folder_name(self):
        props = self.loader._parse_folder_name("10hz_2ob_60s")
        self.assertEqual(props.carrier_frequency, 10.0)
        self.assertEqual(props.target_frequency, 5.0)
        
        with self.assertRaises(ValueError):
            self.loader._parse_folder_name("invalid_folder")
    ...
    @patch('os.path.isdir')
    def test_load_happy_path(self, mock_isdir, mock_exists, mock_listdir):
        # Mock FS
        ...
        mock_mne.find_events.return_value = np.array([[100, 0, 1], [500, 0, 1]])
        ...
        mock_mne.Epochs.return_value = mock_epochs_inst
