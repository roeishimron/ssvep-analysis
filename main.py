...
def zscore_spectrum(psd, noise_n_neighbor_freqs=1, noise_skip_neighbor_freqs=1):
    """Compute SNR spectrum from PSD spectrum using convolution.
    ...
    # Construct a kernel that calculates the mean of the neighboring
    # frequencies
    DEVIATION = noise_n_neighbor_freqs + noise_skip_neighbor_freqs
    averaging_kernel = np.concatenate(
        (
            np.ones(DEVIATION),
            np.ones(1),
    ...
    def into_stds(_psd):
        stds = [0] * DEVIATION
        for center in range(DEVIATION, len(_psd) - DEVIATION):
            stds.append(np.std(_psd[center-DEVIATION:center+DEVIATION]))
        stds += [0] * DEVIATION
        return stds

    # std_kernel = np.full(noise_n_neighbor_freqs * 2 + 2 * noise_skip_neighbor_freqs + 1, True)
    # # Calculate the mean of the neighboring frequencies by convolving with the
    # # averaging kernel.
    stds = np.apply_along_axis(into_stds, axis=-1, arr=psd)

    return (psd - mean_noise) / (stds)
